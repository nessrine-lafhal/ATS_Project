...
import type { ChatbotEvaluation } from "./types"
...
export class EnhancedChatbotService {
  ...
  async getEvaluation(): Promise<ChatbotEvaluation | null> {
    try {
  ...
  private simulateEvaluation(): ChatbotEvaluation {
...
let chatbotServiceInstance: EnhancedChatbotService | null = null

export const getEnhancedChatbotService = (sessionId?: string): EnhancedChatbotService => {
  if (!chatbotServiceInstance || sessionId) {
    chatbotServiceInstance = new EnhancedChatbotService(sessionId)
  }
