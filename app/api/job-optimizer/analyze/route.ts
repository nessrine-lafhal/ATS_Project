...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await JobOptimizerService.analyzeJobDescription(job_description)

    return NextResponse.json({ success: true, data: result })
  } catch (error) {
    console.error("Erreur lors de l'analyse de la description de poste:", error)
    return NextResponse.json({ error: "Erreur lors de l'analyse de la description de poste" }, { status: 500 })
  }
}
